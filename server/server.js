/*
 * Nlag Monitoring Server
 * node "server/server.js"
 * DO NOT require("./server") in other modules, it likely creates circular dependency!
 */
console.log("Welcome to Nlag Monitoring");
...
// Exit Nlag Monitoring immediately if the Node.js version is banned
...
log.info("server", "Nlag Monitoring Version: " + checkVersion.version);
...
(async () => {
    ...
    app.get("/.well-known/change-password", async (_, response) => {
        response.redirect("https://github.com/samarun/uptime-kuma/wiki/Reset-Password-via-CLI");
    });
        ...
        socket.on("prepare2FA", async (currentPassword, callback) => {
            try {
                ...
                if (user.twofa_status === 0) {
                    ...
                    // Related issue: https://github.com/samarun/uptime-kuma/issues/486
        ...
        socket.on("setup", async (username, password, callback) => {
            try {
                ...
                if ((await R.knex("user").count("id as count").first()).count !== 0) {
                    throw new Error("Nlag Monitoring has been initialized. If you want to run setup again, please delete the database.");
                }
...
async function initDatabase(testMode = false) {
    ...
    // If there is no record in user table, it is a new Nlag Monitoring instance, need to setup
    if ((await R.knex("user").count("id as count").first()).count === 0) {
...
let unexpectedErrorHandler = (error, promise) => {
    ...
    console.error("If you keep encountering errors, please report to https://github.com/samarun/uptime-kuma/issues");
};
